...
float K[4]={3.86,   7.3 ,  -0.3398   ,-1.0827};// vector K del controller -4+-j2 -70+-j70
float L[4]={-0.0124 , 0.0134 ,  0.09   , 0.0856};   // vector L del observer
float Ki = 0.35;

float A_fila1[4]={0.4172  ,  0.5451  , -0.0224 ,   0.0021};// matriz A del observador Aodd
float A_fila2[4]={0.0238 ,   0.0567 ,  -0.0017 ,  -0.0004};
float A_fila3[4]={2.8248 , -24.0329  , -0.1074 ,   0.3195};
float A_fila4[4]={4.6734 ,  11.9881  , -0.2824 ,  -0.1164};
...
float Cd[4]={-7.4601   ,94.1305         ,0         ,0};// armado de theta estimada
...
//    matlab_send(angulo);
      ...
      xi += Ts * error;   
      // Opcional: anti-windup: limitamos xi si es demasiado grande
      // Ajusta límites según tu sistema
      if (xi > 50) xi = 50;
      if (xi < -50) xi = -50;
      control_moderno(theta, &theta_e, &u_1,&u_print,xi);
...
void control_moderno(float theta, float* theta_e, float* u_1, float* u_print, float xi)
{
...
xe[3]= (A_fila4[0]*xe_1[0]+ A_fila4[1]*xe_1[1]+A_fila4[2]*xe_1[2]+A_fila4[3]*xe_1[3]) + B_fila4[0]* (*u_1) + B_fila4[1]*theta ;
  ...
  float u = float u = -1*( K[0]*xe[0] + K[1]*xe[1] + K[2]*xe[2] + K[3]*xe[3] + Ki * xi );
