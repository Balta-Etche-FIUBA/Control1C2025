...
const float Kp =0.781;
const float Ki = 7.81;
const float Kd = 0.01;
const float t_d = 1/300; 
...
void loop() {
  ...
  integral = e * Ts;
  // derivativo
  float alpha = Ts / (t_d +Ts);
  float d_e = (e - e_prev) / Ts;
  float  derivative = derivative + alpha * (derivative - d_e);
